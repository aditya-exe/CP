...
class ELOMAX {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            ...
            int M = in.ni();
            int[] a = in.ria(N);
            int[][] rating = new int[N + 1][M + 1];
            int[] max_rating = new int[N + 1];
            int[][] rank = new int[N + 1][M + 1];
            int[] bestrank = new int[N + 1];

            in.r();
            for (int i = 1; i <= N; i++) rating[i][0] = in.ni();
            ...
            for (int j = 1; j <= M; j++) {
                Pair[] ratingID = new Pair[N];
                for (int i = 1; i <= N; i++) {
                    ratingID[i - 1] = new Pair(rating[i][j], i);
                }
                Arrays.sort(ratingID);
                int cur_rank = 1, prev = (int) ratingID[0].first, next_rank = 2;
                for (int i = 0; i < N; i++) {
                    if (prev == (int) ratingID[i].first) {
                        rank[(int) ratingID[i].second][j] = cur_rank;
                        next_rank++;
                    } else {
                        cur_rank = next_rank;
                        next_rank++;
                        rank[(int) ratingID[i].second][j] = cur_rank;
                        prev = (int) ratingID[i].first;
                    }
                }
            }
            ...
            int ans = 0;
            for (int i = 1; i <= N; i++) {
                if (bestrank[i] != max_rating[i]) ans++;
            }
