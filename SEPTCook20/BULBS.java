...
class BULBS {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            ...
            int N = in.ni();
            int K = in.ni();
            StringBuilder sb = new StringBuilder(in.rline());

            ArrayList<Integer> side = new ArrayList<>();
            ArrayList<Integer> zmid = new ArrayList<>();
            int count = 0;
            int count0s = 0;
            ...
            if (count0s != 0) {
                side.add(count0s);
                sb = new StringBuilder(sb.substring(count0s));
            }
            ...
            int total = zmid.stream().mapToInt(x -> x).sum() + side.stream().mapToInt(x -> x).sum();
            zmid.sort(Comparator.reverseOrder());
            ...
            for (int i : zmid) {
                if (K > 2) {
                    ...
                    if (zmid.size() >= 2) {
                        gcuts += Math.max(side.stream().mapToInt(x -> x).sum(), i);
                    } else {
                        gcuts += side.stream().mapToInt(x -> x).sum();
                    }
                }
            }

            out.prln(total - gcuts);
        }
    }
