...
class Team {
    ...
    public Team(String name, int g, int p) {
        this.name = name;
        this.goals = g;
        this.points = p;
    }

    void add(int g, int p) {
        this.goals += g;
        this.points += p;
    }

    public String toString() {
        return "Name: " + name + " Points: " + points + " Goals: " + goals;
    }
}
...
class Match {
    ...
    public Match(String t1, String t2, int p1, int p2, int g1, int g2) {
        this.t1 = t1;
        this.t2 = t2;
        this.p1 = p1;
        this.p2 = p2;
...
class ucl {
    ...
    void main() throws IOException {
        int t = fr.nextInt();
        Match[][] match = new Match[t][12];
        for (int i = 0; i < t; i++) {
            HashMap<String, Team> map = new HashMap<>();
            for (int j = 0; j < 12; j++) {
                String s = fr.nextLine();
                match[i][j] = getMatchData(s);
            }
            for (int j = 0; j < 12; j++) {
                map = setMatchData(match[i][j], map);
            }
            sortingTeams(map);
        }
    }

    public static void sortingTeams(HashMap<String, Team> map) {
        ArrayList<Team> teams = new ArrayList<>();
        for (String key : map.keySet()) {
            teams.add(map.get(key));
        }
        teams.sort(new SortByPoints());
        out.prln(teams.get(3).name + " " + teams.get(2).name);
    }

    public static Match getMatchData(String str) {
        String[] mstr = str.split(" ");
        int g1 = Integer.parseInt(mstr[1]);
        int g2 = Integer.parseInt(mstr[3]);

        if (g1 > g2) {
            return new Match(mstr[0], mstr[4], 3, 0, g1 - g2, g2 - g1);
        } else if (g2 > g1) {
            return new Match(mstr[0], mstr[4], 0, 3, g1 - g2, g2 - g1);
        } else {
            return new Match(mstr[0], mstr[4], 1, 1, g1 - g2, g2 - g1);
        }
    }

    public static HashMap<String, Team> setMatchData(Match m, HashMap<String, Team> map) {
        if (map.containsKey(m.t1)) {
            map.get(m.t1).add(m.g1, m.p1);
        } else {
            map.put(m.t1, new Team(m.t1, m.g1, m.p1));
        }
        if (map.containsKey(m.t2)) {
            map.get(m.t2).add(m.g2, m.p2);
        } else {
            map.put(m.t2, new Team(m.t2, m.g2, m.p2));
        }
        return map;
    }
