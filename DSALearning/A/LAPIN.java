...
class LAPIN {
    ...
    private boolean check(String s) {
        ...
        if (s.length() % 2 == 0) {
            m1 = checkFreq(s.substring(0, s.length() / 2));
            m2 = checkFreq(s.substring(s.length() / 2));
        } else {
            m1 = checkFreq(s.substring(0, (int) s.length() / 2));
            m2 = checkFreq(s.substring((int) s.length() / 2 + 1));
        }
        return m1.equals(m2);
    }

    private Map<Character, Integer> checkFreq(String s) {
