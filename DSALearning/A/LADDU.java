...
class LADDU {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            in.r();
            int activities = in.ni();
            String origin = in.n();
            int total = 0;
            while (activities-- > 0) {
                String[] a = in.rline().split(" ");
                if (a[0].equals("CONTEST_WON"))
                    total += (Integer.parseInt(a[1]) >= 20) ? 300 : 300 + 20 - Integer.parseInt(a[1]);
                if (a[0].equals("TOP_CONTRIBUTOR")) total += 300;
                if (a[0].equals("BUG_FOUND")) total += Integer.parseInt(a[1]);
                if (a[0].equals("CONTEST_HOSTED")) total += 50;
            }
            if (origin.equals("INDIAN")) out.prln(total / 200);
            else out.prln(total / 400);
        }
    }
    ...
    /*static class FastReader {
        ...
        String next() {
            while (st == null || !st.hasMoreElements()) {
                try {
                    st = new StringTokenizer(br.test.readLine());
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
