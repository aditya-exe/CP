...
class EVENT {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            ...
            String start = in.n();
            String end = in.n();
            int L = Integer.parseInt(in.n());
            int R = Integer.parseInt(in.n());
            ...
            days.put("saturday", 6);
            days.put("sunday", 7);
            int diff = (days.get(end) - days.get(start) + 8) % 7;
            int ans = -1;
            int times = 0;
            for (int i = 0; i < 100; i++) {
                if (diff <= R && L <= diff) {
            ...
            if (times > 1) out.prln("many");
            else if (times == 0) out.prln("impossible");
            else out.prln(ans);
        }
    }
