...
public class SUBMEXS {
    ...
    int N;
    long ans;
    ArrayList<Long> SubTree = new ArrayList<>();
    ...
    private long computeSubtree(int node, int parent) {
        SubTree.set(node, (long) 1);
        for (var i : graph.get(node)) {
            if (i == parent) {
                continue;
            }
            SubTree.set(node, SubTree.get(node) + computeSubtree(i, node));
        }
        return SubTree.get(node);
    }

    void dfs(int node, int parent, long sum_of_sub_ancestor) {
        long value = sum_of_sub_ancestor + SubTree.get(node);
        int child = 0;
        for (var i : graph.get(node)) {
            if (i == parent) {
                continue;
