...
class KCHESS {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            int N = in.ri();
            ArrayList<Pair<Integer, Integer>> knights = new ArrayList<>();
            while (N-- > 0) {
                in.r();
                knights.add(new Pair<>(in.ni(), in.ni()));
            }
            in.r();
            int Kx = in.ni();
            int Ky = in.ni();
            Pair<Integer, Integer> KING = new Pair<>(Kx, Ky);
            ArrayList<Pair<Integer, Integer>> KMoves = new ArrayList<>();
            for (Pair<Integer, Integer> p : knights) {
                KMoves.add(new Pair<>(p.first - 1, p.second - 2));
                KMoves.add(new Pair<>(p.first + 1, p.second - 2));
                KMoves.add(new Pair<>(p.first + 2, p.second - 1));
                KMoves.add(new Pair<>(p.first + 2, p.second + 1));
                KMoves.add(new Pair<>(p.first - 1, p.second + 2));
                KMoves.add(new Pair<>(p.first + 1, p.second + 2));
                KMoves.add(new Pair<>(p.first - 2, p.second - 1));
