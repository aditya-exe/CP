...
class CHFNSWAP {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            ...
            long z = n - y;
            outt.println(y * (y - 1) / 2 + z * (z + 1) / 2);
        }
    }

    private long bs(long x, long n) {
        long l = 1, r = n, m, ans = n;
        while (l <= r) {
            m = (l + r) / 2;
            if (m * (m + 1) / 2 <= x) {
                ans = m;
    ...
    private long work(long m) {
        return (long) (((-2.0) + (sqrt(4.0 + 8 * ((double) m * (double) m + (double) m)))) / 4.0);
    }
