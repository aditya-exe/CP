...
class COVID19B {
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            ...
            int best = N;
            int worst = 0;
            SortedMap<Pair<Integer, Integer>, ArrayList<Integer>> map = new TreeMap<>(comparator);
            for (int i = 0; i < N; i++) {
                for (int j = i + 1; j < N; j++) {
                    if (a[i] > a[j]) {
                        int t = (j - i) / (a[i] - a[j]);
                        ...
                        b.add(i);
                        b.add(j);
                        Pair<Integer, Integer> pair = new Pair<>(t, x);
                        map.put(pair, b);
                    }
                }
            }
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (Map.Entry<Pair<Integer, Integer>, ArrayList<Integer>> entry : map.entrySet()) {
                    ...
                    for (int x : entry.getValue()) spread |= infected[x];
                    if (spread) {
    ...
    static class Pair<F, S> implements Comparable<Pair<F, S>> {
        ...
        public int compareTo( Pair pair) {
            if ((int) pair.first < (int) this.first) return 1;
            else if ((int) pair.first > (int) this.second) return -1;
            else return 0;
        }
    }
}