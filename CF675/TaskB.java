...
public class TaskB {
            ...
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < M; j++) {
                    ...
                    if (i <= ax && j <= ay) {
                        ...
                        al.add(a[i][j]);
                        if (ax != i) al.add(a[ax][j]);
                        if (ay != j) al.add(a[i][ay]);
                        if (ax != i && ay != j) al.add(a[ax][ay]);
    ...
    private long solve(ArrayList<Long> al) {
        long ans = LMAX;
        for (Long i : al) {
            int sum = 0;
            for (Long j : al) {
    ...
    static class in {
        ...
        static long[][] rim(int n, int m) throws IOException {
            long[][] a = new long[n][m];
            for (int i = 0; i < n; i++) {
                in.r();
                for (int j = 0; j < m; j++) {
