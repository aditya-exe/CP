...
class WALKFAST {
    ...
    void main() throws IOException {
        int t = fr.nextInt();
        while (t-- > 0) {
            long n = fr.nextLong(), a = fr.nextLong(), b = fr.nextLong(),
                    c = fr.nextLong(), d = fr.nextLong(), p = fr.nextLong(),
                    q = fr.nextLong(), y = fr.nextLong();
            long arr[] = new long[(int) (n + 1)];
            for (long i = 1; i <= n; i++) arr[(int) i] = fr.nextInt();

            long tab = Math.abs(arr[(int) a] - arr[(int) b]) * p;
            long atc = Math.abs(arr[(int) c] - arr[(int) a]) * p;
            long wait = y >= atc ? y - atc : Integer.MAX_VALUE;
            long intrain = Math.abs(arr[(int) d] - arr[(int) c]) * q;
            long dtb = Math.abs(arr[(int) d] - arr[(int) b]) * p;

            long tt = atc + wait + intrain + dtb;

            System.out.println(Math.min(tt, tab));
        }
    }
