...
public class alternatingSum {
    ...
    public static void main(String[] $) throws IOException {
        alternatingSum o = new alternatingSum();
        o.main();
        out.flush();
        out.close();
    }
    ...
    void main() throws IOException {
        ...
        while (T-- > 0) {
            int n = in.ri();
            ArrayList<Integer> nums = in.riaa(n);
            int cnt0 = Collections.frequency(nums, 0);
            int cnt1 = Collections.frequency(nums, 1);

            if (cnt0 >= n / 2) {
                out.prln(cnt0);
                for (int i = 0; i < cnt0; i++) out.pr(0 + " ");
            } else {
                out.prln(cnt1 - cnt1 % 2);
                for (int i = 0; i < cnt1 - cnt1 % 2; i++) out.pr(1 + " ");
            }
            out.prln();
        }
    }
